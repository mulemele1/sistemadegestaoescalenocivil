...
@section('title', 'Lista de desembolsos')
                        ...
                        <div class="card mt-3">
                            <div class="card-header">
                                ...
                                <div class="card-tools">
                                    <a href="{{ route('desembolsos.create') }}"
                                        ...
                                        <tbody>
                                            @foreach ($desembolsos as $desembolso)       
                                                <tr data-id="{{ $desembolso->id }}"
                                                    data-gestao-name="{{ $desembolso->name }}"
                                                    data-projecto-acronimo="{{ $desembolso->acronimo }}"
                                                    data-gerencia-name="{{ $desembolso->nome }}"
                                                    data-valor="{{ $desembolso->valor }}"
                                                    data-status="{{ $desembolso->status }}">
                                                    <td>{{ $desembolso->name }}</td>
                                                    <td>{{ $desembolso->acronimo }}</td>
                                                    <td>{{ $desembolso->nome }}</td>
                                                    <td>{{ number_format($desembolso->valor, 2) }}</td>
                                                    <td>{{ $desembolso->data_desem }}</td>
                                                    <td>
                                                        <span class="status 
                                                    ...
                                                    <td>
                                                        <div class="btn-group" style="gap: 10px;">
                                                            ...
                                                            <a role="button" class="btn bg-lightblue"
                                                                href="{{ route('desembolsos.edit', $desembolso->id) }}">
...
<script>
    ...
    const administracaoName = row.getAttribute('data-administracao-nome'); // Corrigido o nome da variável
